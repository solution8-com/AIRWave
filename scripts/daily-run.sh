#!/usr/bin/env bash
# AIRWave daily run + deploy to GitHub Pages
# Usage: ./scripts/daily-run.sh
# Cron:  0 8 * * * /path/to/airwave/scripts/daily-run.sh >> /path/to/airwave/logs/cron.log 2>&1
...
echo "$LOG_PREFIX Starting AIRWave daily run..."
