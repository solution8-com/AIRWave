"""Application service for staged AIRWave pipeline execution."""
...
class AIRWavePipelineService:
    ...
    def get_run_stage(
        ...
        if max_items <= 0:
            raise AIRWaveMcpError(code="HZ_INVALID_INPUT", message="max_items must be greater than 0.")
        try:
    ...
    def get_effective_config(
        ...
        return {
            "airwave_path": str(ctx.airwave_path),
    ...
    async def fetch_items(
        ...
        if hours <= 0:
            raise AIRWaveMcpError(code="HZ_INVALID_INPUT", message="hours must be greater than 0.")
    ...
    async def score_items(
        ...
        if not items:
            raise AIRWaveMcpError(code="HZ_EMPTY_INPUT", message="No items available for scoring.")
    ...
    async def enrich_items(
        ...
        if not items:
            raise AIRWaveMcpError(code="HZ_EMPTY_INPUT", message="No items available for enrichment.")
    ...
    def _build_context(
        self,
        airwave_path: str | None,
        config_path: str | None,
        sources: list[str] | None,
    ) -> tuple[PipelineContext, list[str], list[str]]:
        resolved_airwave = resolve_airwave_path(airwave_path)
        runtime = load_runtime(resolved_airwave)
        resolved_config = resolve_config_path(resolved_airwave, config_path)
    ...
    def _load_stage_items(
        ...
        ctx, _, _ = self._build_context(airwave_path=airwave_path, config_path=config_path, sources=None)
        try:
